...
void loop() {
  ...
  if (radio.receiveDone())
  {
    ...
    //sprintf(payload_send, "fr:%d,to:%d,ac:%d,msg:%s", to, fr, ac, message);
    sprintf(payload_send, "ac:%d,msg:%s", "1", "to you");
    ...
    if (radio.sendWithRetry(radio.SENDERID, (const void*)(&payload_send), sizeof(payload_send))){
